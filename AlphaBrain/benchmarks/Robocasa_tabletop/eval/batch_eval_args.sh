...
cd ./AlphaBrain
ALPHABRAIN_PYTHON=~/miniconda3/envs/AlphaBrain/bin/python
ROBOCASA_PYTHON=~/miniconda3/envs/robocasa/bin/python
export PYTHONPATH=$(pwd):${PYTHONPATH}
CKPT_DEFAULT="AlphaBrain/Qwen3-VL-OFT-Robocasa/checkpoints/steps_90000_pytorch_model.pt"
...
=======
CKPT_DEFAULT="results/training/1224_fourier_gr1_unified_1000_QwenPI_v2_state_qwen3/checkpoints/steps_180000_pytorch_model.pt"
N_ENVS_DEFAULT=1
...
EvalEnv() {
    ...
    ${ROBOCASA_PYTHON} benchmarks/Robocasa_tabletop/eval/simulation_env.py \
...
for GPU_ID in $(seq 0 $((NUM_GPUS - 1))); do
    ...
    CUDA_VISIBLE_DEVICES=${GPU_ID} \
    ${ALPHABRAIN_PYTHON} deployment/model_server/server_policy.py \
        --ckpt_path "${CKPT_PATH}" \
        --port "${PORT}" \
        --use_bf16 \
...
# 判断是否还有 benchmarks/Robocasa_tabletop/eval/simulation_env.py
while pgrep -f "benchmarks/Robocasa_tabletop/eval/simulation_env.py" > /dev/null; do
